...
class MultivariateEvaluator(Evaluator):
    ...
    def energy_score(self, ts_iterator: Iterator[pd.DataFrame],
                     forecast_iterator: Iterator[Forecast]):

        for time_series, forecast in zip(ts_iterator, forecast_iterator):
            pred_target = np.array(
                self.extract_pred_target(time_series, forecast))
            pred_target = np.ma.masked_invalid(pred_target)
            pred_target = pred_target.transpose()

            # Fix for the case `prediction_length = 1`.
            # This should be done in `extract_pred_target` properly later without affecting univariate models.
            pred_target = np.atleast_2d(pred_target)

            num_timesteps = pred_target.shape[0]
            # print('pred', pred_target.shape, forecast.samples.shape)
            all_timestep_scores = [
                MultivariateEvaluator.
                    energy_score_per_timestep(pred_target[t], forecast.samples[:, t, :])
                    # energy_score_per_timestep(pred_target[t],
                    #                           forecast.samples[:, :, t])
                for t in range(num_timesteps)
            ]

        return np.array(all_timestep_scores).mean()

    @staticmethod
    def energy_score_per_timestep(y: np.array, samples: np.array) -> float:
        m = samples.shape[0]
        # print('y', y.shape, 'samples', samples.shape)
        assert y.shape[0] == samples.shape[1]

        s1 = np.linalg.norm(y - samples, axis=1).sum()
        s2 = 0
        for i in range(m):
            for j in range(i, m):
                s2 += 2 * np.linalg.norm(samples[i, :] - samples[j, :])
    ...
    def __call__(
        ...
        # print('eval_dims: ', eval_dims)

        ts_iterator_set = tee(
            ts_iterator, target_dimensionality + len(self.target_agg_funcs) + 1
        )
        fcst_iterator_set = tee(
            fcst_iterator, target_dimensionality + len(self.target_agg_funcs) + 1
        )
        
        # print('ts_iterator_Set: ', ts_iterator_set)
        # print(type(ts_iterator_set))
        # print(len(ts_iterator_set))
        ...
        for dim in eval_dims:
            ...
            # isFirst or print('extrct trgt : ', type(ts_iterator_set[dim]))
            # isFirst or print(list(ts_iterator_set))
            # isFirst or print('extrct forcst : ', type(fcst_iterator_set[dim]))
            # isFirst or print(list(fcst_iterator_set))
            # isFirst = False
        ...
        # print("ts_iteratorset:::")
        # for item in ts_iterator_set:
        #     print(ts_iterator_set)
        #     print(list(ts_iterator_set))
        #     print("..........")

        # print("fcst_iterator_set:::")
        # for item in fcst_iterator_set:
        #     print(fcst_iterator_set)
        #     print(list(fcst_iterator_set))
        #     print("..........")

        if self.target_agg_funcs:
            multivariate_metrics = {
                agg_fun_name: self.calculate_aggregate_multivariate_metrics(
                    ts_iterator_set[-(index + 2)],
                    fcst_iterator_set[-(index + 2)],
                    agg_fun,
                )
        ...
        last_it_timeseries = ts_iterator_set[-1]
        last_it_forecast = fcst_iterator_set[-1]
        es = self.energy_score(last_it_timeseries, last_it_forecast)
        all_agg_metrics['energy_score'] = es
